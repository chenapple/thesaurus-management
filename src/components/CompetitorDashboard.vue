...
import type { CompetitorTask, CompetitorRun, CompetitorAsin } from '../types';
import { getCountryLabel, getCountryFlag } from '../types';

// Emits
const emit = defineEmits<{
  (e: 'runTask', task: CompetitorTask): void;
  (e: 'editTask', task: CompetitorTask): void;
}>();

// 数据状态
const loading = ref(false);
const tasks = ref<CompetitorTask[]>([]);
const recentRuns = ref<CompetitorRun[]>([]);

// 抽屉相关
const drawerVisible = ref(false);
const selectedTask = ref<CompetitorTask | null>(null);
const taskRuns = ref<CompetitorRun[]>([]);
const taskAsins = ref<CompetitorAsin[]>([]);
...
async function loadData() {
  ...
  try {
    const [tasksResult, runsResult] = await Promise.all([
      invoke<CompetitorTask[]>('get_competitor_tasks'),
      invoke<CompetitorRun[]>('get_latest_competitor_runs', { limit: 20 }),
    ]);
...
const tasksByMarketplace = computed(() => {
  const groups = new Map<string, CompetitorTask[]>();
...
// 打开任务详情抽屉
async function openTaskDrawer(task: CompetitorTask) {
  selectedTask.value = task;
  drawerVisible.value = true;
  loadingRuns.value = true;

  try {
    const [runs, asins] = await Promise.all([
      invoke<CompetitorRun[]>('get_competitor_runs_by_task', { taskId: task.id, limit: 20 }),
      invoke<CompetitorAsin[]>('get_competitor_asins', { taskId: task.id }),
    ]);
    taskRuns.value = runs;
    taskAsins.value = asins;
    if (runs.length > 0) {
...
// 删除任务
async function handleDeleteTask(task: CompetitorTask) {
  try {
    await ElMessageBox.confirm(
      `确定要删除任务「${task.name}」吗？所有相关数据将被删除。`,
      '删除确认',
...
// 运行任务
function handleRunTask(task: CompetitorTask) {
  emit('runTask', task);
  drawerVisible.value = false;
}

// 编辑任务
function handleEditTask(task: CompetitorTask) {
  emit('editTask', task);
}
  ...
  <div class="dashboard-container" v-loading="loading">
      ...
      <div class="modern-card stat-card">
        <div class="stat-top">
          <div class="icon-circle bg-purple-light">
            <el-icon class="text-purple"><Aim /></el-icon>
          </div>
          <span class="stat-title">监控任务</span>
        </div>
        <div class="stat-main">
      ...
      >
        ...
        <div class="task-grid">
          ...
          >
            ...
            <div class="task-meta">
              <div class="task-asin-hint" v-if="task.my_asin">
                对比: {{ task.my_asin }}
              </div>
              <div class="task-time">
    ...
    <div v-else class="empty-state">
      <el-empty description="暂无监控任务">
        <template #default>
          <p class="empty-hint">在「执行」页面创建监控任务后，可以在这里查看所有竞品的动态变化</p>
        </template>
      </el-empty>
    </div>
    ...
    >
        ...
        <div class="drawer-header">
          ...
          <div class="drawer-actions">
            ...
            <el-button
              ...
              type="danger"
              plain
              @click="handleDeleteTask(selectedTask!)"
            >
              删除
            </el-button>
          </div>
      ...
      <div class="drawer-content" v-loading="loadingRuns">
        <!-- 监控的竞品 -->
        <div v-if="taskAsins.length > 0" class="asins-section">
          <div class="section-title">监控竞品 ({{ taskAsins.length }})</div>
          <div class="asins-list">
            <el-tag v-for="asin in taskAsins" :key="asin.id" type="info" class="asin-tag">
              {{ asin.asin }}
            </el-tag>
          </div>
        </div>

...
### 商品描述要求
1. **必须生成2个版本**的商品描述
2. **纯文本格式**：不要使用 Markdown 语法（如 #、**、- 等），只输出纯文本，用换行符分段
3. **结构建议**：品牌故事/产品定位 → 核心卖点 → 使用场景 → 规格/保障
4. **关键词埋入**：自然融入2-3个关键词（与标题/五点不重复的词优先）
5. **字符限制**：控制在1000-2000字符（英文）或500-1000字符（中文）
6. **突出差异化**：强调竞品评论中的痛点解决方案
...
\`\`\`json
{
  ...
  "description_suggestions": [
    {
      "version": 1,
      "content": "完整的商品描述文案（支持换行符\\n分段）",
      "structure": "结构说明：品牌故事+核心卖点+使用场景",
      "embedded_keywords": ["埋入的关键词1", "埋入的关键词2"],
      "highlights": ["突出卖点1", "突出卖点2", "突出卖点3"],
      "reason": "为什么这样写，以及关键词选择理由"
    }
  ],
  "aplus_suggestions": {
    "main_image": {
